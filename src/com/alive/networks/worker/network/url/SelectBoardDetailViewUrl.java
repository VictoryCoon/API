...
public class SelectBoardDetailViewUrl extends RequestEngine {

	private static Logger logger = LoggerFactory.getLogger(SelectBoardDetailViewUrl.class);
	...
	public String workProcess(String jsonString, ChannelHandlerContext ctx) throws Exception {
		...
		if (vo == null) {
			...
			List<ApiInfoVo> boardDetail = apiDao.selectBoardDetailView(vo);
			List<ApiInfoVo> boardContentList = apiDao.selectBoardContentList(vo);

			if (boardDetail != null) {
				resMap.put("code",ResponseCode.OK);
				resMap.put("desc",ResponseCode.OK_DESC);
				resMap.put("boardDetail", boardDetail);
				logger.info(boardContentList.get(0).toString());
				if(boardContentList != null){
					resMap.put("boardContentList", boardContentList);
				}else {
					resMap.put("boardContentList", "");
				}
