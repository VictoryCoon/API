...
public class SelectReReplyListUrl extends RequestEngine {

	private static Logger logger = LoggerFactory.getLogger(SelectReReplyListUrl.class);
	...
	public String workProcess(String jsonString, ChannelHandlerContext ctx) throws Exception {
		...
		try {
			...
			vo.setBoardReplySid(reqParam.get("boardReplySid").toString());
		} catch (Exception e) {
		...
		if (vo == null) {
			...
			if(vo.getBoardReplySid() == null){
				usrRes.setCode(ResponseCode.EUI0_CODE);
				usrRes.setDesc(ResponseCode.EUI0_DESC+" : vo.getBoardReplySid()");
				return makeResponseString(usrRes);
			}
			if(vo.getBoardReplySid().length() > 24){
				usrRes.setCode(ResponseCode.EUI1_CODE);
				usrRes.setDesc(ResponseCode.EUI1_DESC+" : vo.getBoardReplySid()");
				return makeResponseString(usrRes);
			}

			List<ApiInfoVo> boardReReReplyList = apiDao.selectBoardReReplyList(vo);

			if (boardReReReplyList != null) {
				resMap.put("code",ResponseCode.OK);
				resMap.put("desc",ResponseCode.OK_DESC);
				resMap.put("boardReReReplyList", boardReReReplyList);
			} else {
